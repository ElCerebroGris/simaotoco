$(function () {
  ...
  $("html").on("submit", "form.form-wizard", function (e) {
    ...
    $.ajax({
      url: "http://localhost:8000/sgemst/membro/request",
      type: "POST",
      data: formData,
      cache: false,
      processData: false,
      ...
      success: function (data) {
        ...
        if (data.error) {
          $(".error")
            .html('<p class="alert alert-danger">' + data.errMessage + "</p>")
            .fadeIn(200);
        } else {
          $(".error").html("").fadeOut();
        }
        ...
        if (data.finish) {
          //console.log();
          location.replace("http://localhost:8000/sgemst/membro/listar");
        }
      },
      error: function (data) {
        console.log(data);
